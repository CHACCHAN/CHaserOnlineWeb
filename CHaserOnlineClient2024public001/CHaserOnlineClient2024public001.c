/**********************************************
	CHaserOnlineClient2024public_001
...
int send_cmd(char *command, char *param, char *returnCode){
	...
	if(strcmp(SessionID, "")==0){
		sprintf(send_buf, "GET http://%s/CHaserOnline003/user/%s?%s HTTP/1.1\r\n", host, command, param);
	}
	else{
		sprintf(send_buf, "GET http://%s/CHaserOnline003/user/%s;jsessionid=%s?%s HTTP/1.1\r\n", host, command, SessionID, param);
	}
	...
	sprintf(send_buf, "User-Agent:CHaserOnlineClient/2024\r\n");
	write(s, send_buf, strlen(send_buf));
